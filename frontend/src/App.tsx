...
const nav: Array<{ name: View; icon: typeof Activity }> = [{name:'Dashboard',icon:Activity},{name:'Market',icon:BarChart3},{name:'Strategy',icon:Database},{name:'Risk Guard',icon:ShieldAlert},{name:'Paper Trading',icon:WalletCards},{name:'Backtest',icon:TestTube2},{name:'Trade Journal',icon:BookOpen},{name:'Settings',icon:Settings}]
...
function App() {
  ...
  const paper=usePaperReadOnly(), backtest=useBacktest()
  const requestKey=`${selection.source}:${selection.symbol}:${selection.timeframe}:${refreshKey}`
  const loading=loadedRequest!==requestKey

  useEffect(()=>{ let mounted=true; Promise.all([tradingGuardApi.getMarketCandles(selection.symbol,selection.timeframe,100,selection.source),tradingGuardApi.getIndicators(selection.symbol,selection.timeframe,100,selection.source),tradingGuardApi.getStrategy(selection.symbol,selection.timeframe,100,selection.source)]).then(([candles,indicators,strategies])=>{if(!mounted)return;setData({candles,indicators,strategies});setError(null);setLoadedRequest(requestKey)}).catch((cause:unknown)=>{if(!mounted)return;setData({candles:[],indicators:[],strategies:[]});setError(cause instanceof Error?cause.message:'Unable to load selected market source.');setLoadedRequest(requestKey)});return()=>{mounted=false}},[selection,refreshKey,requestKey])

  const candle=data.candles.at(-1), indicator=data.indicators.at(-1), strategy=data.strategies.at(-1), real=selection.source==='binance'
  const controls=<div className="panel-soft flex flex-wrap items-end gap-3 rounded p-4"><label className="text-xs"><span className="mono mb-1 block text-[9px] text-slate-500">SOURCE</span><select value={selection.source} onChange={event=>setSelection(value=>({...value,source:event.target.value as MarketSource}))} className="rounded border border-slate-700 bg-slate-900 px-3 py-2"><option value="binance">REAL · Binance Public</option><option value="mock">MOCK · Deterministic</option></select></label><label className="text-xs"><span className="mono mb-1 block text-[9px] text-slate-500">SYMBOL</span><select value={selection.symbol} onChange={event=>setSelection(value=>({...value,symbol:event.target.value}))} className="rounded border border-slate-700 bg-slate-900 px-3 py-2"><option>BTCUSDT</option><option>ETHUSDT</option><option>BNBUSDT</option></select></label><label className="text-xs"><span className="mono mb-1 block text-[9px] text-slate-500">TIMEFRAME</span><select value={selection.timeframe} onChange={event=>setSelection(value=>({...value,timeframe:event.target.value}))} className="rounded border border-slate-700 bg-slate-900 px-3 py-2">{['1m','5m','15m','1h','4h','1d'].map(timeframe=><option key={timeframe}>{timeframe}</option>)}</select></label><button disabled={loading} onClick={()=>setRefreshKey(value=>value+1)} className="flex items-center gap-2 rounded border border-sky-500/30 bg-sky-500/10 px-3 py-2 text-xs text-sky-300 disabled:opacity-50"><RefreshCw size={14} className={loading?'animate-spin':''}/>{loading?'Loading…':'Refresh'}</button></div>
  ...
  const content:Record<View,ReactNode>={
    ...
    Market:<div className="space-y-4"><MarketChart candles={data.candles} indicators={data.indicators} symbol={selection.symbol} timeframe={selection.timeframe}/><MarketView candles={data.candles} symbol={selection.symbol}/></div>,
    ...
    'Trade Journal':<TradeJournalView paper={paper}/>, Settings:<SettingsView selection={selection}/>,
  }
