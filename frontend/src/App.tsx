...
function App(){
 ...
 const candle=data.candles.at(-1),completedCandle=data.candles.at(-2),indicator=data.indicators.at(-1),strategy=data.strategies.at(-1),real=source==='binance'
 const authoritativeStrategy=paperActive&&autoCycle?.strategy?autoCycle.strategy:strategy

 // Autonomous PAPER is keyed to the same completed candle the backend consumes. The currently
 // forming market candle may change every refresh, but it is display-only and cannot retrigger entry.
 useEffect(()=>{
   if(!paperActive||!completedCandle?.timestamp||loading)return
   const key=`${source}:${symbol}:${timeframe}:${completedCandle.timestamp}`
   ...
   return()=>{active=false}
 },[paperActive,completedCandle?.timestamp,loading,source,symbol,timeframe,paperRefresh])
