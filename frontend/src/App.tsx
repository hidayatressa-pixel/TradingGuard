...
function App(){
 ...
 const candle=data.candles.at(-1),indicator=data.indicators.at(-1),strategy=data.strategies.at(-1),real=source==='binance'

 // Autonomous PAPER driver: UI only triggers a backend-owned cycle. It sends no strategy,
 // reference price or stop. Duplicate 5-second refreshes are suppressed per market candle.
 useEffect(()=>{
   if(!paper.account?.active){lastAutoRequest.current='';setAutoCycle(null);setAutoCycleError(null);return}
   if(!strategy?.timestamp||loading)return
   const key=`${source}:${symbol}:${timeframe}:${strategy.timestamp}`
   if(lastAutoRequest.current===key)return
   lastAutoRequest.current=key
   let active=true
   void tradingGuardApi.autoPaperCycle(symbol,timeframe,source,0.5,20).then(result=>{
     if(!active)return
     setAutoCycle(result);setAutoCycleError(null);void paper.refresh()
   }).catch((cause:unknown)=>{
     if(!active)return
     setAutoCycleError(cause instanceof Error?cause.message:'Siklus Auto Paper gagal.')
   })
   return()=>{active=false}
 },[paper.account?.active,strategy?.timestamp,loading,source,symbol,timeframe,paper.refresh])
