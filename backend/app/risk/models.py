...
class RiskPolicy(BaseModel):
    ...
    def validate_policy(self) -> "RiskPolicy":
        numeric = (
            self.max_risk_per_trade_pct, self.warning_risk_per_trade_pct,
            self.max_daily_loss_pct, self.warning_daily_loss_pct,
            self.max_total_exposure_pct, self.warning_total_exposure_pct,
            self.max_drawdown_pct, self.warning_drawdown_pct,
        )
        if not all(math.isfinite(value) for value in numeric):
            raise ValueError("Risk policy numeric values must be finite")
        ...
        if self.max_risk_per_trade_pct <= 0: raise ValueError("max_risk_per_trade_pct must be positive")
        if self.max_daily_loss_pct <= 0: raise ValueError("max_daily_loss_pct must be positive")
        if self.max_total_exposure_pct <= 0: raise ValueError("max_total_exposure_pct must be positive")
        if self.max_open_positions <= 0: raise ValueError("max_open_positions must be positive")
        if self.max_drawdown_pct <= 0: raise ValueError("max_drawdown_pct must be positive")
        return self
...
class RiskContext(BaseModel):
    ...
    def validate_context(self) -> "RiskContext":
        numeric = (self.risk_per_trade_pct, self.daily_loss_pct, self.total_exposure_pct, self.current_drawdown_pct)
        if not all(math.isfinite(value) for value in numeric):
            raise ValueError("Risk context numeric values must be finite")
        if self.risk_per_trade_pct < 0: raise ValueError("risk_per_trade_pct must be non-negative")
        if self.daily_loss_pct < 0: raise ValueError("daily_loss_pct must be non-negative")
        if self.total_exposure_pct < 0: raise ValueError("total_exposure_pct must be non-negative")
        if self.open_positions < 0: raise ValueError("open_positions must be non-negative")
        if self.current_drawdown_pct < 0: raise ValueError("current_drawdown_pct must be non-negative")
        return self
