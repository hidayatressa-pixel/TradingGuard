...
class PaperTradingService:
    ...
    def execute_manual_guarded_entry(self,*,strategy:StrategyResult,risk:RiskResult,quantity:float,stop_loss_price:float,execution_price:float,timestamp:datetime)->PaperAccount:
        """Execute a MANUAL_GUARDED paper BUY immediately after an authoritative gate.

        This path is intentionally separate from autonomous next-candle scheduling: it exists
        to test the reader/guard interactively. Risk Guard ALLOW and bullish strategy alignment
        are still mandatory; there is no manual bypass.
        """
        account=self.state(); self._roll_risk_day(timestamp)
        if not account.active or not account.config.paper_trading_enabled: raise ValueError("Paper trading is inactive.")
        if account.open_position is not None or account.pending_entry is not None or account.pending_exit is not None: raise ValueError("Paper account is not flat.")
        ...
        account.open_position=PaperPosition(symbol=strategy.symbol,timeframe=strategy.timeframe,entry_signal_timestamp=strategy.timestamp,entry_timestamp=timestamp,entry_price=effective_price,quantity=quantity,entry_notional=entry_notional,entry_transaction_cost=entry_fee,entry_assessment=strategy.assessment.value,cash_before_entry=cash_before,cash_after_entry=account.cash,stop_loss_price=stop_loss_price,entry_mode="MANUAL_GUARDED")
        account.total_transaction_cost+=entry_fee; return account

    def execute_manual_exit(self,*,execution_price:float,timestamp:datetime)->PaperAccount:
        """Close an existing paper position immediately; reducing exposure never needs ALLOW."""
        account=self.state(); position=account.open_position
        if not account.active or position is None: raise ValueError("There is no active paper position to SELL.")
        if execution_price<=0: raise ValueError("Manual exit price must be positive.")
        effective_price=float(execution_price)*(1.0-account.config.slippage_pct/100.0)
        self._close_position(timestamp=timestamp,effective_price=effective_price,exit_signal_timestamp=timestamp,exit_assessment="MANUAL_SELL",exit_reason=PaperExitReason.MANUAL)
        return account
    ...
    def _close_position(self,*,timestamp:datetime,effective_price:float,exit_signal_timestamp:datetime,exit_assessment:str,exit_reason:PaperExitReason)->None:
        ...
        account.closed_trades.append(PaperTrade(symbol=position.symbol,timeframe=position.timeframe,entry_signal_timestamp=position.entry_signal_timestamp,entry_timestamp=position.entry_timestamp,entry_price=position.entry_price,exit_signal_timestamp=exit_signal_timestamp,exit_timestamp=timestamp,exit_price=effective_price,quantity=position.quantity,entry_notional=position.entry_notional,exit_notional=exit_notional,gross_pnl=gross_pnl,entry_transaction_cost=position.entry_transaction_cost,exit_transaction_cost=exit_fee,transaction_cost=total_fee,net_pnl=net_pnl,return_pct=(net_pnl/committed_capital)*100.0,equity_before=equity_before,equity_after=account.realized_equity,entry_assessment=position.entry_assessment,exit_assessment=exit_assessment,exit_reason=exit_reason,entry_mode=position.entry_mode)); account.total_transaction_cost+=exit_fee; account.open_position=None; account.pending_exit=None; account.peak_realized_equity=max(account.peak_realized_equity,account.realized_equity)
