...
class PendingEntry(BaseModel):
    ...
    action: PendingActionType = PendingActionType.ENTRY
...
class PendingExit(BaseModel):
    ...
    action: PendingActionType = PendingActionType.EXIT
    signal_timestamp: datetime
    symbol: str
    timeframe: str
    assessment: str
...
class PaperPerformanceSnapshot(BaseModel):
    ...
    pending_action: PendingEntry | PendingExit | None
...
class PaperAccount(BaseModel):
    ...
    def validate_state(self) -> "PaperAccount":
        if self.pending_entry is not None and self.pending_exit is not None:
            raise ValueError("paper account cannot have pending entry and exit actions together.")
        for value in (self.initial_capital, self.cash, self.realized_equity, self.total_transaction_cost):
            if not math.isfinite(value):
                raise ValueError("paper account numeric values must be finite.")
        return self

    @property
    def pending_action(self) -> PendingEntry | PendingExit | None:
        return self.pending_entry or self.pending_exit
    ...
    def performance(self) -> PaperPerformanceSnapshot:
        ...
        total = len(trades)
        average = sum(trade.net_pnl for trade in trades) / total if total else 0.0
        return PaperPerformanceSnapshot(
            initial_capital=self.initial_capital,
            realized_equity=self.realized_equity,
            realized_net_profit=self.realized_equity - self.initial_capital,
            realized_return_pct=((self.realized_equity - self.initial_capital) / self.initial_capital) * 100.0,
            total_closed_trades=total,
            winning_trades=winning,
            losing_trades=losing,
            breakeven_trades=total - winning - losing,
            win_rate_pct=(winning / total) * 100.0 if total else 0.0,
