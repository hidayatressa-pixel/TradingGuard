...
class ManualGuardedTradeService:
    """Interactive PAPER decision tester.

    BUY is never a bypass: completed-candle strategy must be bullish, Auto Stop must
    be valid, sizing/context must be available, and Risk Guard must return ALLOW.
    SELL reduces exposure and therefore does not require an ALLOW decision.
    """
    ...
    def buy(self, *, completed_candles: list[Candle], strategy: StrategyResult,
        ...
        if account.open_position is not None or account.pending_entry is not None or account.pending_exit is not None:
            return self._result("BUY", False, "BLOCK", strategy, account, reason="Account is not flat; close/cancel the current paper action first.")
        ...
        sizing = RiskSizingRequest(
            equity=float(account.realized_equity), entry_price=float(market_price),
            stop_loss_price=float(stop.stop_loss_price), risk_budget_pct=float(risk_budget_pct),
        ...
        account = self.paper_service.execute_manual_guarded_entry(
            strategy=strategy, risk=gate.risk, quantity=gate.sizing.final_quantity,
            stop_loss_price=gate.sizing.stop_loss_price, execution_price=float(market_price),
            timestamp=datetime.now(timezone.utc),
        )
        return self._result("BUY", True, "ALLOW", strategy, account, gate=gate, auto_stop=stop,
                            execution_price=float(account.open_position.entry_price) if account.open_position else None,
                            reason="Manual PAPER BUY executed immediately after Strategy + Auto Stop + Risk Sizing + Risk Guard ALLOW.")

    def sell(self, *, strategy: StrategyResult, market_price: float) -> ManualGuardedTradeResult:
        account = self.paper_service.state()
        if not account.active or account.open_position is None:
            return self._result("SELL", False, "BLOCK", strategy, account, reason="No active paper position to SELL.")
        account = self.paper_service.execute_manual_exit(execution_price=float(market_price), timestamp=datetime.now(timezone.utc))
        return self._result("SELL", True, "REDUCE_RISK", strategy, account, execution_price=float(market_price),
                            reason="Manual PAPER SELL executed; closing exposure does not require Risk Guard ALLOW.")

    @staticmethod
    def _result(action: Literal["BUY", "SELL"], executed: bool, decision: str,
                strategy: StrategyResult, account: PaperAccount, *,
                gate: ProspectiveRiskGateResult | None = None,
                auto_stop: AutoStopLossResult | None = None,
                execution_price: float | None = None, reason: str) -> ManualGuardedTradeResult:
        return ManualGuardedTradeResult(action=action, executed=executed, decision=decision,
                                        strategy=strategy, gate=gate, auto_stop=auto_stop,
